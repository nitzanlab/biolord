...
from ._base_components import DistributionDecoderWrapper, NormalDecoderWrapper, RegularizedEmbedding
from ._constants import LOSS_KEYS

__all__ = ["BiolordModule", "BiolordClassifyModule"]
...
class BiolordModule(BaseModuleClass):
    ...
    multi_decoder
        Define a multi-decoder model, a full decoder per modality.
    decoder_width
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.px_r = {}
        if multi_decoder:
            self.joint_decoder = nn.Identity()
            DistributionDecoder = DecoderSCVI
            NormalDecoder = Decoder
        else:
            ...
            DistributionDecoder = DistributionDecoderWrapper
            NormalDecoder = NormalDecoderWrapper

        for gene_likelihood, x_loc_, n_var in zip(gene_likelihoods, x_locs, n_vars):
            if gene_likelihood in ["nb", "poisson"]:
                self.decoders[x_loc_] = DistributionDecoder(
                ...
                self.decoders[x_loc_] = NormalDecoder(
                    n_input=self.n_decoder_input,
                    n_output=n_var,
                    n_hidden=decoder_width,
                    n_layers=decoder_depth,
    ...
    def generative(
        ...
        for i, x_loc_ in enumerate(self.x_locs):
            if self.gene_likelihoods[i] in ["nb", "poisson"]:
                ...
                p_m, p_v = self.decoders[x_loc_](x=z)
